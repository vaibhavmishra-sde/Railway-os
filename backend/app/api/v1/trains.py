...
def create_train_endpoint(
    payload: TrainCreate, db: Session = Depends(get_db)  # noqa: B008
) -> TrainResponse:
    try:
...
@router.post("/{train_id}/coaches", response_model=CoachResponse, status_code=status.HTTP_201_CREATED)
def add_coach_endpoint(
    train_id: str, payload: CoachCreate, db: Session = Depends(get_db)  # noqa: B008
) -> CoachResponse:
